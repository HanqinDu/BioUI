...
<form action="QiimeQualityControlPairListener" method="post" enctype="multipart/form-data" onsubmit="disabledSubmit();">
...
Forward read trim position (5') (INTEGER)
<span class="wrapper">[?]
    <div class="tooltip">Position at which forward read sequences should be trimmed due to low quality. This trims the 5' end of the input sequences, which will be the bases that were sequenced in the first cycles.</div>
</span>
: <input type="text" name="p-trim-left-f" value="0">
<br>

Reverse read trim position (5') (INTEGER)
<span class="wrapper">[?]
    <div class="tooltip">Position at which reverse read sequences should be trimmed due to low quality. This trims the 5' end of the input sequences, which will be the bases that were sequenced in the first cycles.</div>
</span>
: <input type="text" name="p-trim-left-r" value="0">
<br>


*Forward read truncate position (3') (INTEGER) 
<span class="wrapper">[?]
    <div class="tooltip">Position at which forward read sequences should be truncated due to decrease in quality. This truncates the 3' end of the of the input sequences, which will be the bases that were sequenced in the last cycles. Reads that are shorter than this value will be discarded. After this parameter is applied there must still be at least a 12 nucleotide overlap between the forward and reverse reads. If 0 is provided, no truncation or length filtering will be performed</div>
</span>
: <input type="text" name="p-trunc-len-f" value="0">
<br>

*Reverse read truncate position (3') (INTEGER) 
<span class="wrapper">[?]
    <div class="tooltip">Position at which reverse read sequences should be truncated due to decrease in quality. This truncates the 3' end of the of the input sequences, which will be the bases that were sequenced in the last cycles. Reads that are shorter than this value will be discarded. After this parameter is applied there must still be at least a 12 nucleotide overlap between the forward and reverse reads. If 0 is provided, no truncation or length filtering will be performed</div>
</span>
: <input type="text" name="p-trunc-len-r" value="0">
...
Minimum overlap required for merging (INTEGER)
<span class="wrapper">[?]
    <div class="tooltip">The minimum length of the overlap required for merging the forward and reverse reads.</div>
</span>
: <input type="text" name="p-min-overlap" value="">
...
<h4 id="extra-parameters">DADA2 Extra parameters</h4>
<pre><code>  <span class="hljs-comment">--p-max-ee-f NUMBER    Forward reads with number of expected errors higher</span>
                         than this <span class="hljs-built_in">value</span> will be discarded.     [default: <span class="hljs-number">2.0</span>]
  <span class="hljs-comment">--p-max-ee-r NUMBER    Reverse reads with number of expected errors higher</span>
                         than this <span class="hljs-built_in">value</span> will be discarded.     [default: <span class="hljs-number">2.0</span>]
