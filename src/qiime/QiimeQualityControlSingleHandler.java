...
public class QiimeQualityControlSingleHandler extends TaskHandler{
	
	private String method = "";
	
	private String pooling_method = "";
	private String p_chimera_method = "";
	private String p_trunc_len = "0";
	private String p_trim_left = "0";
	...
	public QiimeQualityControlSingleHandler(HttpServletRequest request,
	...
	protected void onReceive(Part part) throws IOException {
		switch(part.getName()){
			...
			case "p-trunc-len":
				p_trunc_len = loadInt(part, 0, 100000, "0"); break;
			case "p-trim-left":
				p_trim_left = loadInt(part, 0, 100000, "0"); break;
			case "p-trunc-q":
				p_trunc_q = loadInt(part, 0, 100000, ""); break;
			case "p-min-overlap":
				p_min_overlap = loadInt(part, 4, 100000, ""); break;
	...
	protected boolean onValidateInput(){
		...
		if(method.equals("dada2")){
			if(p_trunc_len.isEmpty()){
				setErrorReport("sequence read truncate position (3') cannot be empty");
				flag = false;
			}
		}
	...
	protected void onSetCommand() throws IOException {
		...
		if(method.equals("dada2")){
			combineParameters = addFlag(pooling_method, "--p-pooling-method") +
				addFlag(p_trunc_len, "--p-trunc-len") + addFlag(p_trim_left, "--p-trim-left") +
				addFlag(p_trunc_q, "--p-trunc-q") + addFlag(p_min_overlap, "--p-min-overlap") +
				addFlag(p_chimera_method, "--p-chimera-method") + parameters;

				combineParameters = "dada2 denoise-single --p-n-threads 0 --i-demultiplexed-seqs import/rawdata.qza --o-table quality_control/treated_table.qza --o-representative-sequences quality_control/treated_rep_seqs.qza --o-denoising-stats quality_control/treated_stats.qza " + combineParameters;
		}
		...
		String[] command1 = {"sh", Config.QIIME_QC_SINGLE_SH, combineParameters};
		String[] command2 = {"mkdir", "-p", Config.RESULT_DIR + File.separator + taskName};
		String[] command3 = {"cp", "quality_control/dada2_treated_stats.qzv", "quality_control/dada2_treated_rep_seqs.qzv", "quality_control/dada2_treated_table.qzv", "quality_control/sample_metadata.tsv", "log_error.txt", "log.txt", Config.RESULT_DIR + File.separator + taskName};
		...
		setResultMessage((hyperlink("treated_rep seqs.qzv", Config.RESULT_URL + File.separator + taskName + File.separator +  "treated_rep_seqs.qzv")));
